...
_realloc macro
    push ax
    push bx
    mov sp, programLength + 100h + 200h ; смещение sp на 200 после конца программы
    mov ax, programLength + 100h + 200h
    shr ax, 4                           ; ибо должны помещаться блоки памяти по 16(параграфы) делим на 16
    inc ax
    mov bx, ax
    mov ah, 4Ah                         ; изменяем размер блока памяти для новой программы
    int 21h
    pop bx
    pop ax
endm
